...
class SimplePreProcess():
    NORM_PX = 2000  # the short side must have this resolution
...
class AdvancedPreProcess():
    def resize_to_screen(self, src, norm_w=1280, norm_h=700, copy=False):
    ...
    def assemble_spans(self, small, pagemask, cinfo_list):
        # sort list
    ...
    def keypoints_from_samples(self, small, pagemask, page_outline, span_points):
    ...
    def process(self, img):
        ...
        spans = self.assemble_spans(small, pagemask, cinfo_list)
        
        if len(spans) < 3:
            cinfo_list = self.get_contours(small, pagemask, 'line')
            spans2 = self.assemble_spans(small, pagemask, cinfo_list)
        ...
        corners, ycoords, xcoords = self.keypoints_from_samples(small, pagemask, page_outline, span_points)
